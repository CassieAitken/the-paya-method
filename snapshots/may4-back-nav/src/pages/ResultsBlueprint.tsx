...
function getMasterPillarBridge(score: number, label: string, name: string, pPoss: string, pillarId: string): string {
  const bridges: Record<string, Record<string, string>> = {
    ...
    sync: {
      ...
      low: `A decade in — and the pack bond still has <strong>untapped depth</strong>. ${name} has spent years calibrating to your signal. What ${pPoss} nervous system needs now isn't more love — it's more consistent, intentional co-regulation. The rituals below unlock what's been waiting.`,
        ...
        <div className="bg-white/80 backdrop-blur-md border border-stone-200/70 p-10 lg:p-12 shadow-[0_15px_50px_-12px_rgba(0,0,0,0.08)] rounded-xl">
          ...
          <div className="space-y-6">
            ...
            {track === 'legacy' ? (
              ...
              <p className="text-stone-800 leading-[1.8] font-light">
                What you described is not a feeling — it's a measurable biological phenomenon called <strong>interspecies autonomic co-regulation</strong>, and after a decade, the circuitry runs deep. {Name}'s nervous system has been reading yours for years — the micro-tensions, the breath patterns, the moments you let go. {pSubj.charAt(0).toUpperCase() + pSubj.slice(1)} feel{pVerb === 'is' ? 's' : ''} it all. At this stage, that read is <strong>faster and more precise</strong> than it's ever been. Which means your grounded state triggers deeper repair — and your scattered state disrupts more.
              </p>
            ) : (
              <p className="text-stone-800 leading-[1.8] font-light">
      ...
      <div className="space-y-12 pt-12 border-t border-stone-200/40">
        <div className="border-b border-stone-200/40 pb-8">
          ...
          <p className="text-stone-700 text-base mt-4 leading-[1.8] font-light max-w-3xl">
            {track === 'legacy'
              ? <>We analyzed all <strong>81 markers</strong> and isolated the rituals that will bring {Name} the <strong>most comfort and spark</strong> in this golden chapter. Ordered by what matters most now. Gentle shifts. Meaningful impact.</>
              : track === 'master'
                ? <>We analyzed all <strong>81 markers</strong> and isolated the rituals with the <strong>highest leverage</strong> for {Name}'s profile at this stage. After a decade, every optimization carries more weight. Ordered by biological impact. Precision over volume.</>
                : <>We analyzed all <strong>81 markers</strong> and isolated the rituals with the <strong>highest leverage</strong> for {Name}'s profile. Ordered by biological impact. Start at the top. Work down.</>
            }
          </p>
        </div>
      ...
      <div className="space-y-12 pt-12 border-t border-stone-200/40">
        <div>
          ...
          <p className="text-stone-700 text-sm mt-4 leading-[1.8] font-light">
            All 81 markers across all seven pillars. Your permanent reference — revisit this as you implement the protocol.
          </p>
        </div>
