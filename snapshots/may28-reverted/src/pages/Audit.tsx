...
const SOULFUL_SCALE_OPTIONS: { text: string; subtitle: string; value: FrequencyLevel }[] = [
  { text: "Resilient", subtitle: "A steady, natural rhythm of ease", value: "resilient" },
  { text: "Whispering", subtitle: "Occasional or seasonal signs of subtle noise", value: "whispering" },
  { text: "Burdened", subtitle: "Systemic fatigue calling for deep, active clearing", value: "burdened" },
];
...
export function Audit({
    ...
    return (
      <div className="min-h-screen bg-[#FAF9F5] flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center animate-[fade-in_0.4s_ease-out]">
          <div className="w-16 h-16 rounded-full bg-[#415A42]/10 flex items-center justify-center mx-auto mb-6">
            <Icons.Check size={28} strokeWidth={2} className="text-[#415A42]" />
          </div>
          <p className="text-[9px] font-mono uppercase tracking-[0.4em] text-[#415A42] mb-3">
          ...
          <p className="text-sm text-[#4A4A4A]/70 font-light leading-relaxed mb-8">
            {PHASE_CELEBRATIONS[completedPhaseId]}
          </p>
          <div className="border-t border-stone-200/40 pt-6 mb-6">
            <p className="text-[9px] font-mono uppercase tracking-[0.3em] text-[#4A4A4A]/40 mb-1">Up Next</p>
            <p className="text-lg font-serif text-[#2A2421]">{AUDIT_PHASES[nextPhaseId]?.name}</p>
            <p className="text-xs text-[#4A4A4A]/50 font-light mt-1">{AUDIT_PHASES[nextPhaseId]?.desc}</p>
          </div>
          <button
            onClick={() => {
            ...
            className="inline-flex items-center gap-2 px-8 py-3.5 bg-[#2A2421] text-white rounded-full text-sm font-medium hover:bg-[#1a1715] transition-colors"
          >
            Continue
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F5] flex flex-col">
      ...
      <div className="sticky top-0 z-30 bg-[#FAF9F5]/95 backdrop-blur-sm border-b border-stone-200/40 px-4 py-3">
        <div className="max-w-2xl mx-auto">
                ...
                <div
                  ...
                  className={`flex-1 flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-mono uppercase tracking-widest whitespace-nowrap transition-all duration-300 ${
                    isActive
                      ...
                      : isComplete
                      ? 'bg-[#415A42]/10 text-[#415A42] border border-[#415A42]/30'
                      : 'bg-white text-[#4A4A4A]/60 border border-stone-200/60'
                  }`}
                >
                  <span className="hidden sm:inline truncate">{AUDIT_PHASES[phaseId].name}</span>
                  <span className="sm:hidden">Phase {phaseId}</span>
                  {isComplete && <Icons.Check size={8} strokeWidth={3} className="text-[#415A42]" />}
                </div>
              );
            })}
          </div>
          <div className="flex items-center gap-3">
            <div className="flex-1 h-1.5 bg-stone-200/60 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#415A42] rounded-full transition-all duration-500"
        ...
        <div className="w-full max-w-2xl">
          ...
          <div className="mb-6 sm:mb-8">
            <div className="flex items-center gap-2 mb-3">
              <span className="flex items-center gap-1.5 text-[9px] font-mono uppercase tracking-[0.35em] text-[#4A4A4A]/60">
                <CurrentPhaseIcon size={11} strokeWidth={1.5} />
                Phase {currentPhaseId} of 3 — {AUDIT_PHASES[currentPhaseId].name}
              </span>
            </div>
            <div className="bg-[#2A2421] text-white rounded-3xl p-5 sm:p-7 shadow-[0_8px_30px_rgb(0,0,0,0.025)]">
          ...
          >
            ...
            {/* Question text */}
            <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.025)] border border-stone-100 p-6 sm:p-8 mb-4">
            ...
            <div className="space-y-2.5">
              {SOULFUL_SCALE_OPTIONS.map((option) => {
                const isSelected = currentFrequency === option.value;
                return (
                  <button
                    key={option.value}
                    onClick={() => handleSelect(option.value)}
                    className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl border-2 transition-all duration-150 text-left group ${
                      isSelected
                        ? 'border-[#415A42] bg-[#415A42]/5 shadow-[0_8px_30px_rgb(0,0,0,0.025)]'
                        : 'border-stone-200/60 bg-white hover:border-stone-300/80 hover:shadow-[0_8px_30px_rgb(0,0,0,0.025)]'
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                        isSelected
                          ? 'border-[#415A42] bg-[#415A42]'
                      ...
                      <div>
                        ...
                        <p className={`text-xs font-light mt-0.5 transition-colors ${
                          isSelected ? 'text-[#4A4A4A]' : 'text-[#4A4A4A]/50'
                        }`}>
                          {option.subtitle}
                        </p>
                      </div>
                    ...
                    {isSelected && (
                      <Icons.Check size={16} strokeWidth={2.5} className="text-[#415A42] flex-shrink-0" />
                    )}
                  </button>
                );
              })}
          ...
          {/* Navigation row */}
          <div className="flex items-center justify-between mt-6 pt-5 border-t border-stone-200/40">
            <button
              onClick={() => advanceQuestion('back')}
              className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-[0.3em] text-[#4A4A4A]/50 hover:text-[#2A2421] transition-colors py-2"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-mono uppercase tracking-[0.3em] font-medium transition-all ${
                  canAdvanceManually
                    ? 'bg-[#415A42] text-white hover:bg-[#344a35] hover:-translate-y-0.5 shadow-[0_8px_30px_rgb(0,0,0,0.025)]'
                    : 'bg-stone-100 text-[#4A4A4A]/30 cursor-not-allowed'
                }`}
              >
          ...
          <div className="mt-8 pl-4 border-l-2 border-[#415A42]/20">
            <p className="text-[10px] font-mono uppercase tracking-[0.3em] text-[#4A4A4A]/40 mb-1.5 flex items-center gap-1.5">
              <Icons.Microscope size={10} strokeWidth={1.5} /> Biological Insight
            </p>
            <p className="text-xs text-[#4A4A4A]/60 leading-[1.7] font-light">
              {foundation.insight}
            </p>
          </div>
        </div>
