...
function App() {
  ...
  const [dogNumber, setDogNumber] = useState<number | null>(null);
      ...
      fetch(completeUrl, {
        ...
        .then(data => {
          if (data.success && data.purchaseData) {
            ...
            if (data.dogNumber) setDogNumber(data.dogNumber);
  ...
  const handleShare = async () => {
    ...
    const shareText = `${dogName} just scored ${results.score}/100 on the Dog Biology Blueprint™.
...
${dogName} scored ${results?.score || ''}/100 on the Dog Biology Blueprint™ — 50 carefully selected markers, 7 Biology Markers, one personalized blueprint for their long-term health.
