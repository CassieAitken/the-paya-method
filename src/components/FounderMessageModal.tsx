...
export function FounderMessageModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-black/50" onClick={onClose}>
      <div className="relative bg-[#FDFBF7] rounded-none shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-8 lg:p-12 space-y-8">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-px bg-[#9AB8C4]"></div>
                <p className="text-[10px] uppercase tracking-[0.2em] text-[#9AB8C4] font-medium">From the Founder</p>
              </div>
              <h3 className="text-4xl lg:text-5xl font-serif text-[#2A2A28] leading-tight">
                A Message From<br />the Founder
              </h3>
            </div>
            <button
              onClick={onClose}
              className="ml-4 p-2 hover:bg-[#E8E2D9]/50 rounded-lg transition-colors flex-shrink-0 mt-1"
            >
              <Icons.X size={22} className="text-[#8A8A86]" />
            </button>
          </div>
          ...
          <div className="space-y-6 text-[#5C534E] leading-[1.8] font-light">
            ...
            <div className="pt-6 border-t border-[#E8E2D9]">
              <p className="text-lg font-serif text-[#2A2A28]">Cassie Aitken</p>
              <p className="text-[10px] text-[#9AB8C4] uppercase tracking-[0.15em] mt-1 font-medium">Founder, The Paya Method</p>
              <ul className="mt-3 space-y-1.5">
                {[
                  ...
                  <li key={item} className="flex items-center gap-2 text-xs text-[#8A8A86] font-light">
